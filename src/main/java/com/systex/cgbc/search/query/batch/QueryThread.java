...
public class QueryThread implements Runnable {
     ...
     */
    public QueryThread(ThreadPoolExecutor _queryThreadPool, List<Rule> _rules,
    ...
    private String response(JSONObject requestEntity) throws Exception {
        SingleQueryThread t =
            new SingleQueryThread(queryAPI, requestEntity, rules, precisyCount, fuzzyyCount,
                nonCount);
    ...
    public void run() {
        ...
        while (true) {
            ...
            if (requestEntity != null) {
                ...
                LOG.info("单笔查询时间:" + elapsed + " ms");

                // 2，不停的去任务队列里面去取任务
                requestEntity = requestQueue.poll();
                // 前台输出，便于查看进度
                if (resultQueue.size() % 500 == 0) {
                    LOG.info(DateUtil.getNowTime() + ":" + resultQueue.size());
                }
        ...
        LOG.info(Thread.currentThread().getName() + " has done!");
    }
}
